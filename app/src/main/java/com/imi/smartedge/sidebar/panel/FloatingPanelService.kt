...
class FloatingPanelService : Service() {
    ...
    companion object {
        ...
        const val ACTION_OPEN = BuildConfig.APPLICATION_ID + ".OPEN"
        const val ACTION_OPEN_HUB = BuildConfig.APPLICATION_ID + ".OPEN_HUB"
        const val ACTION_REFRESH = BuildConfig.APPLICATION_ID + ".REFRESH"
        const val ACTION_CLOSE_PANEL = BuildConfig.APPLICATION_ID + ".CLOSE_PANEL"
        const val ACTION_SHOW_TEMP = BuildConfig.APPLICATION_ID + ".SHOW_TEMP"
        const val ACTION_TOGGLE = BuildConfig.APPLICATION_ID + ".TOGGLE"
        const val ACTION_SCREENSHOT = BuildConfig.APPLICATION_ID + ".SCREENSHOT"
        const val ACTION_UPDATE_IMMERSIVE = BuildConfig.APPLICATION_ID + ".UPDATE_IMMERSIVE"
        const val ACTION_TOGGLE_FLASHLIGHT = BuildConfig.APPLICATION_ID + ".TOGGLE_FLASHLIGHT"
        const val ACTION_LAUNCH_CAMERA = BuildConfig.APPLICATION_ID + ".LAUNCH_CAMERA"
        const val ACTION_TOGGLE_ROTATION = BuildConfig.APPLICATION_ID + ".TOGGLE_ROTATION"
        const val ACTION_OPEN_FAV_APP = BuildConfig.APPLICATION_ID + ".OPEN_FAV_APP"
    }

    private var notificationUnsubscribe: (() -> Unit)? = null
    ...
    override fun onCreate() {
        ...
        serviceScope.launch {
            if (!panelPrefs.hasConfiguredPanelApps) {
        ...
        notificationUnsubscribe?.invoke()
        notificationUnsubscribe = NotificationTrackingService.subscribe {
            if (isPanelOpen) {
                refreshApps()
            }
        }
    ...
    private fun toggleAutoRotation() {
        try {
            if (!android.provider.Settings.System.canWrite(this)) {
                showIndicator(getString(R.string.ui_requires_write_settings_permission))
    ...
    private fun addEdgeHandle(forceRecreate: Boolean = false) {
        ...
        val hasActiveEngine = PanelCapabilities.canStart(this)
            ...
            val apps = if (currentFolderId != null) {
                when (currentFolderId) {
                    "smartedge.folder.tools" -> {
                        ...
                        tools.add(AppInfo("smartedge.tool.screenshot", getString(R.string.tool_screenshot_short), type = AppInfo.Type.TOOL))
                        
                        // Add Volume tools
                        tools.add(AppInfo("smartedge.tool.volume_up", getString(R.string.ui_volume_plus), type = AppInfo.Type.TOOL))
                        tools.add(AppInfo("smartedge.tool.volume_down", getString(R.string.ui_volume_minus), type = AppInfo.Type.TOOL))
                        
                        // Add Brightness tools
                        tools.add(AppInfo("smartedge.tool.brightness_up", getString(R.string.ui_brightness_plus), type = AppInfo.Type.TOOL))
                        tools.add(AppInfo("smartedge.tool.brightness_down", getString(R.string.ui_brightness_minus), type = AppInfo.Type.TOOL))
                ...
                if (panelPrefs.showToolsPanelButton) {
                    val toolsBtn = AppInfo("smartedge.tool.tools", getString(R.string.ui_tools), type = AppInfo.Type.TOOL)
    ...
    private fun buildNotification(): android.app.Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .addAction(android.R.drawable.ic_menu_view,
                getString(R.string.ui_open_sidebar), openPending)
    ...
    private fun initDragOverlay() {
        ...
        tvTopZone = createZone(getString(R.string.ui_top_split), Gravity.TOP).apply {
            layoutParams.height = (resources.displayMetrics.heightPixels * 0.28).toInt()
        }
        tvBottomZone = createZone(getString(R.string.ui_bottom_split), Gravity.BOTTOM).apply {
            layoutParams.height = (resources.displayMetrics.heightPixels * 0.28).toInt()
        }
        tvFreeformZone = createZone(getString(R.string.ui_freeform_window), Gravity.CENTER).apply {
    ...
    fun adjustVolume(delta: Int) {
        ...
        showIndicator(getString(R.string.volume_percent, percent))
    }
    ...
    fun adjustBrightness(delta: Int) {
        ...
        try {
            if (!android.provider.Settings.System.canWrite(this)) {
                android.widget.Toast.makeText(this, getString(R.string.ui_requires_write_system_settings_permission), android.widget.Toast.LENGTH_SHORT).show()
                val intent = Intent(android.provider.Settings.ACTION_MANAGE_WRITE_SETTINGS).apply {
                    data = android.net.Uri.parse("package:$packageName")
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }
            ...
            showIndicator(getString(R.string.brightness_percent, percent))
        } catch (e: Exception) {
