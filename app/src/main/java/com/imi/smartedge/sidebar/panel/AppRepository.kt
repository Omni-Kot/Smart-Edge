...
class AppRepository(context: Context) {
    ...
    private val appContext = LocaleHelper.onAttach(context.applicationContext)
    private val packageManager: PackageManager = appContext.packageManager
    private val panelPrefs = PanelPreferences(appContext)
    private val iconPackManager = IconPackManager(appContext)

    companion object {
        // A byte budget bounds memory independently of display density and app count.
        private val iconCache = object : android.util.LruCache<String, android.graphics.drawable.Drawable>(16 * 1024 * 1024) {
            override fun sizeOf(key: String, value: android.graphics.drawable.Drawable): Int =
                (value as? android.graphics.drawable.BitmapDrawable)?.bitmap?.allocationByteCount
                    ?: (value.intrinsicWidth.coerceAtLeast(1) * value.intrinsicHeight.coerceAtLeast(1) * 4).coerceAtLeast(1024)
        }
    ...
    fun getProcessedIcon(packageName: String, appearanceKey: String): android.graphics.drawable.Drawable? {
        ...
        val processedIcon = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && rawIcon is android.graphics.drawable.AdaptiveIconDrawable) {
            try {
                ...
                // Rasterize at the displayed icon size, bounded on very high-density screens.
                val size = (72 * density).toInt().coerceIn(72, 384)
        ...
        identifiers.mapNotNull { id ->
            when {
                id == "smartedge.shortcut.one_hand" -> {
                    AppInfo(id, appContext.getString(R.string.ui_one_handed_mode), true, AppInfo.Type.SHORTCUT, appearanceKey = panelPrefs.appearanceKey)
                }
                ...
                id.startsWith("intent:") -> {
                    try {
                        ...
                        val name = resolveInfo?.loadLabel(packageManager)?.toString() 
                                   ...
                                   ?: appContext.getString(R.string.ui_unknown_activity)
