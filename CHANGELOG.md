...
### Smart Edge Fork v.1.4.0_15 (Latest)

- Extracted interface text into resource files and established a scalable structure for additional translations.
- Added the Russian translation and corrected the Spanish resources.
- Registered the application picker as an internal activity route.
- Replaced the quick settings tile launch path with an immutable PendingIntent and removed the obsolete broadcast.
- Added validated settings backup v2 with atomic import and complete user preference coverage.
- Moved Root/Shizuku process execution off the UI thread with timeouts, bounded output, and deterministic cleanup.
- Marked unsupported notch gestures and one-handed mode as unavailable instead of reporting false success.
- Centralized accessibility, automation, and overlay capability checks.
- Made notification subscriptions, callbacks, and package snapshots lifecycle-safe.
- Limited the icon cache to 16 MiB and removed unrestricted global preloading.
- Added adaptive icon rasterization and restored standard RecyclerView caching behavior.
- Extracted freeform window launching into a shared component.
- Guarded hidden API access by Android version and limited the bypass to the required API surface.
- Set the fork application ID to `tech.a8g.fork.smartedge.sidebar.panel` so it can be installed alongside the upstream application.
...
### v1.3.6 (Latest upstream version before the fork)
